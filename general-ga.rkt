...
(define (initialize create-fn strlen popsize)
    ...
    (if (= n 0)
      ...
      (let ([candidate (create-fn strlen)])
        (if (hash-has-key? acc-dict candidate)
          ...
          (f n acc-dict)
          (f (- n 1) (hash-set acc-dict candidate null))))))
    ...
    (let* ([fits      (map fitness-fn pop)]
      ...
      (begin (display curr-best) (newline)
             ...
             (if (>= (car curr-best) cutoff)
               ...
               (if use-elitism
                 (loop (elitism (cdr curr-best) (create-next-gen pop fits popsize strlen)) curr-best)
                 (loop (create-next-gen pop fits popsize strlen) curr-best))))))

  ; Initialize population of individuals
  (let ([init-pop (initialize create-fn strlen popsize)]
